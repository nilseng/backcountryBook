...
router.get("/peaks/:searchTerm", async (req: any, res) => {
    const peaks = await db.peaks.find({ name: { $regex: req.params.searchTerm, $options: "i" } }).limit(5).toArray()
    res.status(200).json(peaks)
})
...
router.post("/peak", checkJwt, async (req: any, res) => {
    ...
    try {
        const doc = await db.peaks.findOneAndUpdate({ _id: new ObjectID(_id) }, { $set: props }, { upsert: true, returnOriginal: false })
