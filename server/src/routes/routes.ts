...
router.post("/route/geojson", checkJwt, async (req: any, res: any) => {
  ...
  const uploadRes = await s3.upload(uploadParams).promise().catch(e => ({ error: e }))
  return uploadRes as aws.S3.ManagedUpload.SendData ? res.status(200).json("Geojson sucessfully uploaded") : res.status(500).json('Geojson upload failed.')
});
